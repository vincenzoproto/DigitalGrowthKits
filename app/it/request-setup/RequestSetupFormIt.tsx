...
export default function RequestSetupFormIt(){
 ...
 function fallback(){const subject=`GuestFlow Systems - richiesta setup ${product}`;const body=[`Sistema: ${product}`,`Referral: ${referral||"diretto"}`,`Struttura: ${propertyName}`,`Tipo: ${propertyType}`,`Camere/unità: ${rooms||"-"}`,`Contatto: ${contactName}`,`Email: ${email}`,`Sito: ${website||"-"}`,`PMS / strumenti: ${currentStack||"-"}`,"",`Note: ${notes||"-"}`].join("\n");window.location.href=`mailto:info@vincenzoproto.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;}
 async function submit(e:FormEvent){e.preventDefault();setSending(true);setStatus("");try{const res=await fetch("/api/setup-request",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({product,referral,propertyName,propertyType,contactName,email,website,rooms,currentStack,databaseSize:"",notes})});if(res.ok){setStatus("Richiesta ricevuta. Valuteremo struttura, stack attuale e obiettivo prima di definire il perimetro dell’implementazione.");}else{setStatus("L’invio diretto non è ancora disponibile: apro una mail già compilata con gli stessi dati.");setTimeout(fallback,350);}}catch{setStatus("Apro una mail già compilata con la tua richiesta.");setTimeout(fallback,350);}finally{setSending(false);}}
 return <form onSubmit={submit} className="setup-form">
   ...
   {status&&<div className="setup-status success" role="status">{status}</div>}
