...
export default function SiteShell({ locale, children }: { locale: "it" | "en"; children: React.ReactNode }) {
  ...
  return <>
    ...
    <footer>
      ...
      <div className="footer-links">{systems.map(([slug, name]) => <Link key={slug} href={`${prefix}/${slug}`}>{name}</Link>)}<Link href={`${prefix}/weak-date-planner`}>{it ? "Planner date deboli" : "Weak-date planner"}</Link><Link href={it ? "/it/partner" : "/partners"}>{it ? "Diventa partner" : "Become a partner"}</Link><Link href={`${prefix}/request-setup`}>{it ? "Richiedi setup" : "Request setup"}</Link><Link href={`${prefix}/privacy`}>Privacy</Link><Link href={`${prefix}/terms`}>{it ? "Termini" : "Terms"}</Link><a href="mailto:info@vincenzoproto.com">{it ? "Contatti" : "Contact"}</a></div>
